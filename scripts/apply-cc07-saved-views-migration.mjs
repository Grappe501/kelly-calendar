/**
 * Apply CC-07 saved-views SQL when migrate deploy history is diverged.
...
const existing = await p.$queryRawUnsafe(`
  ...
  WHERE migration_name = '20260722140000_cc07_saved_views_query_contract'
`);
if (!Array.isArray(existing) || existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'cc07-manual', NOW(), '20260722140000_cc07_saved_views_query_contract', NULL, NULL, NOW(), 1)
  `);
...
await p.$disconnect();
console.log("CC-07 migration applied.");
