/**
 * Apply D21 Communications Provider Dispatch SQL when migrate history diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720210000_v21_communications_provider_dispatch/migration.sql",
  ),
  "utf8",
);
...
console.log(`Communications dispatch SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720210000_v21_communications_provider_dispatch'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d21-dispatch', NOW(), '20260720210000_v21_communications_provider_dispatch', NULL, NULL, NOW(), 1)
  `);
