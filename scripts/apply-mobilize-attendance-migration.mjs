/**
 * Apply D18 Mobilize attendance read SQL when migrate deploy history is diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720180000_v21_mobilize_attendance_read/migration.sql",
  ),
  "utf8",
);
...
console.log(`Mobilize attendance SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720180000_v21_mobilize_attendance_read'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d18-mobilize-attendance', NOW(), '20260720180000_v21_mobilize_attendance_read', NULL, NULL, NOW(), 1)
  `);
