/**
 * Apply D24 audience SQL when migrate history diverged.
...
const migrationName = "20260721010000_v21_communications_audience_recipient_resolution";
...
console.log(`Audience SQL applied (${statements.length} statements).`);
...
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d24-audience', NOW(), '${migrationName}', NULL, NULL, NOW(), 1)
  `);
