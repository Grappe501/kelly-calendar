/**
 * Apply D17 Mobilize event publishing SQL when migrate deploy history is diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720170000_v21_mobilize_event_publishing/migration.sql",
  ),
  "utf8",
);
...
console.log(`Mobilize publishing SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720170000_v21_mobilize_event_publishing'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d17-mobilize-publish', NOW(), '20260720170000_v21_mobilize_event_publishing', NULL, NULL, NOW(), 1)
  `);
