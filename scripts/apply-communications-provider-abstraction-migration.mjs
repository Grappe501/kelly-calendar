/**
 * Apply D22 Communications Provider Abstraction SQL when migrate history diverged.
...
const migrationName = "20260720220000_v21_communications_provider_abstraction";
...
console.log(`Communications provider abstraction SQL applied (${statements.length} statements).`);
...
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d22-provider', NOW(), '${migrationName}', NULL, NULL, NOW(), 1)
  `);
