/**
 * Apply D16 Mobilize foundation SQL when migrate deploy history is diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720160000_v21_mobilize_integration_foundation/migration.sql",
  ),
  "utf8",
);
...
console.log(`Mobilize foundation SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720160000_v21_mobilize_integration_foundation'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d16-mobilize', NOW(), '20260720160000_v21_mobilize_integration_foundation', NULL, NULL, NOW(), 1)
  `);
