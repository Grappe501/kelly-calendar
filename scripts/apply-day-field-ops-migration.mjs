/**
 * Apply only D13 field-ops SQL when migrate deploy history is diverged.
 * Gate: KCCC_ALLOW_SCHEMA_MIGRATION=1
 * Additive / non-destructive. Creates zero fabricated field-ops rows.
 */
...
const sqlPath = path.join(
  root,
  "prisma/migrations/20260720130000_v21_mission_field_ops/migration.sql",
);
...
const before = await p.$queryRawUnsafe(`
  ...
  WHERE table_schema = 'kelly_calendar' AND table_name = 'MissionFieldOpsSession'
`);
console.log("MissionFieldOpsSession table present before:", before[0]?.c > 0);
...
console.log(`Field Ops SQL applied (${statements.length} statements).`);

const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720130000_v21_mission_field_ops'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d13-field-ops', NOW(), '20260720130000_v21_mission_field_ops', NULL, NULL, NOW(), 1)
  `);
...
const sessions = await p.missionFieldOpsSession.count();
const confirmations = await p.missionFieldItemConfirmation.count();
const fieldAcks = await p.missionFieldOpsAcknowledgement.count();
  ...
  JSON.stringify(
    {
      ...
      logisticsAcks,
      fieldOpsSessions: sessions,
      fieldConfirmations: confirmations,
      fieldOpsAcks: fieldAcks,
      fabricatedFieldOps: sessions + confirmations + fieldAcks,
    },
    null,
    2,
  ),
