/**
 * Apply D20 Campaign Communications SQL when migrate deploy history is diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720200000_v21_campaign_communications_queue/migration.sql",
  ),
  "utf8",
);
...
console.log(`Campaign communications SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720200000_v21_campaign_communications_queue'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d20-comms', NOW(), '20260720200000_v21_campaign_communications_queue', NULL, NULL, NOW(), 1)
  `);
