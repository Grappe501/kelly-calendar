/**
 * Apply D19 Volunteer Staffing SQL when migrate deploy history is diverged.
...
const sql = readFileSync(
  path.join(
    root,
    "prisma/migrations/20260720190000_v21_volunteer_staffing/migration.sql",
  ),
  "utf8",
);
...
console.log(`Volunteer staffing SQL applied (${statements.length} statements).`);
const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720190000_v21_volunteer_staffing'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d19-staffing', NOW(), '20260720190000_v21_volunteer_staffing', NULL, NULL, NOW(), 1)
  `);
