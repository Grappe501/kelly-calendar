/**
 * Apply only D11 travel movement SQL when migrate deploy history is diverged.
 * Gate: KCCC_ALLOW_SCHEMA_MIGRATION=1
 * Additive / non-destructive. Creates zero fabricated travel rows.
 */
...
const sqlPath = path.join(
  root,
  "prisma/migrations/20260720110000_v21_mission_travel_movement/migration.sql",
);
...
const before = await p.$queryRawUnsafe(`
  ...
  WHERE table_schema = 'kelly_calendar' AND table_name = 'MissionTravelPlan'
`);
console.log("MissionTravelPlan table present before:", before[0]?.c > 0);
...
console.log(`Travel movement SQL applied (${statements.length} statements).`);

const existing = await p.$queryRawUnsafe(`
  SELECT migration_name FROM kelly_calendar._prisma_migrations
  WHERE migration_name = '20260720110000_v21_mission_travel_movement'
`);
if (existing.length === 0) {
  await p.$executeRawUnsafe(`
    ...
    VALUES
      (gen_random_uuid()::text, 'manual-d11-travel', NOW(), '20260720110000_v21_mission_travel_movement', NULL, NULL, NOW(), 1)
  `);
...
const acks = await p.missionTravelAcknowledgement.count();
