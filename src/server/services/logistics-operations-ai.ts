...
import {
  logisticsOperationsForAdvisory,
...
const ADVISORY_SYSTEM = [
  "You are an advisory logistics operations assistant for Kelly Calendar.",
  "Answer only from the JSON: travel/venue/materials blockers, Unknown fleet facts.",
  "Never treat Unknown as ready. Do not invent vehicles, bookings, or ETAs.",
  "Remind that mission readiness is the minimum of required domains. Under 80 words.",
].join(" ");

export async function maybeGenerateLogisticsOperationsAdvisory(input: {
  ...
  if (!apiKey) {
    return {
      ...
      text: null,
      uncertaintyNote:
        "AI advisory unavailable. Deterministic logistics readiness remains authoritative.",
      provider: null,
    };
  }

  const payload = logisticsOperationsForAdvisory(input.logistics);
  ...
  try {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Logistics operations JSON:\n${JSON.stringify(payload)}`,
          },
        ],
      }),
    });
    ...
    if (!text) {
      return {
        ...
        text: null,
        uncertaintyNote:
          "AI advisory empty. Deterministic logistics readiness remains authoritative.",
        provider: "openai",
      };
    }
    ...
    return {
      ...
      text,
      uncertaintyNote:
        "Advisory only — domain minimum readiness is decided deterministically.",
      provider: "openai",
    };
  } catch {
    ...
    return {
      ...
      text: null,
      uncertaintyNote:
        "AI advisory unavailable (timeout/network). Deterministic logistics remains authoritative.",
      provider: "openai",
    };
  } finally {
    clearTimeout(timer);
...
async function safeAudit(
  ...
  try {
    await writeAttributedAudit({
      actor: input.actor,
      action,
      entityType: "LogisticsOperations",
      requestId: input.requestId,
      source: "logistics-operations-ai",
