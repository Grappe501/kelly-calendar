...
import { archiveMissionIncident } from "@/server/services/mission-incident-service";
...
export async function POST(request: Request, context: Ctx) {
  ...
  return withAuthenticatedMutation(
    request,
    "/api/missions/[missionId]/incidents/[incidentId]/archive",
    async ({ actor }) => {
      await authorize(actor, missionId, incidentId);
      const body = await request.json().catch(() => null);
      return {
        model: await archiveMissionIncident({ incidentId, actor, body }),
      };
    },
  );
}
