...
import { linkFollowUpAction } from "@/server/services/mission-incident-service";
...
export async function POST(request: Request, context: Ctx) {
  ...
  return withAuthenticatedMutation(
    request,
    "/api/missions/[missionId]/incidents/[incidentId]/link-follow-up",
    async ({ actor }) => {
      await authorize(actor, missionId, incidentId);
      const body = await request.json().catch(() => null);
      return {
        model: await linkFollowUpAction({ incidentId, actor, body }),
      };
    },
  );
}
