...
import { acknowledgeIncidentIssue } from "@/server/services/mission-incident-service";
...
export async function POST(request: Request, context: Ctx) {
  ...
  return withAuthenticatedMutation(
    request,
    "/api/missions/[missionId]/incidents/[incidentId]/acknowledgements",
    async ({ actor }) => {
      await authorize(actor, missionId, incidentId);
      const body = await request.json().catch(() => null);
      const result = await acknowledgeIncidentIssue({ incidentId, actor, body });
      return { model: result, created: result.ackCreated };
    },
  );
}
