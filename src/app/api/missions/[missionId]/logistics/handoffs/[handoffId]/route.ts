...
type Ctx = { params: Promise<{ missionId: string; handoffId: string }> };
...
export async function PATCH(request: Request, context: Ctx) {
  const { missionId, handoffId } = await context.params;
  return withAuthenticatedMutation(
    request,
    "/api/missions/[missionId]/logistics/handoffs/[handoffId]",
    async ({ actor }) => {
      await authorize(actor, missionId);
      const body = await request.json().catch(() => null);
      return {
        model: await upsertHandoff({ missionId, actor, handoffId, body }),
      };
    },
  );
}
