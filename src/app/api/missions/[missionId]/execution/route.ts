...
} from "@/server/services/mission-execution-service";

export const dynamic = "force-dynamic";
type Ctx = { params: Promise<{ missionId: string }> };

async function authorizeMissionExec(
...
export async function GET(request: Request, context: Ctx) {
  ...
  return withAuthenticatedQuery(
    ...
    async ({ actor }) => {
      await authorizeMissionExec(actor, missionId, false);
      return getExecuteWorkspace(missionId, actor);
    },
  );
}
...
export async function PATCH(request: Request, context: Ctx) {
  ...
  return withAuthenticatedMutation(
    ...
    "/api/missions/[missionId]/execution",
    async ({ actor }) => {
      await authorizeMissionExec(actor, missionId, true);
      const body = await request.json().catch(() => null);
      return patchMissionExecution(missionId, actor, body);
    },
  );
}
